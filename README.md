...
自分のサーバーに置いて使える、シンプルなURL短縮サービスです（オープンソース）。**設置した人が自分のサービスとして使えるように作ってあります**。サイト名・運営者名・配色・書体・アイコン・利用規約・プライバシーポリシーは、設置後に管理画面から変更できます（コードの書き換えは不要です）。自分や身内だけで使う「限定モード」にもできます。
...
| サブドメイン | 役割 |
|---|---|
| `example.com` | トップページ（発行フォーム）／短縮URLへのアクセス受付（`example.com/abc1234`）／削除用トークンによる削除・利用規約・プライバシーポリシー・お問い合わせ（既定は `/delete` `/terms` `/privacy` `/contact`。管理画面で変更可） |
...
| 機能 | 内容 |
|---|---|
| 発行 | ランダムコード（7桁）。ログインするとカスタムスラッグ（3〜20文字。管理者は1文字から）・無期限も使える。パスワード保護（表示切り替え・12文字の自動生成・安全度の目安つき）・有効期限（未ログインは最大30日。選ぶと「いつまで有効か」を表示）・QRコード。発行とコピーはアニメーションと効果音つき |
| 利用制限 | ログイン: 60件/月・5回/分、未ログイン: 5件/月・3分に1回（IP 単位）。未ログインの発行は reCAPTCHA（スコアベース）で確認 |
| リダイレクト | 中間ページ → `redirect.example.com` で移動先を表示し Safe Browsing で確認 → 自動で移動（下記）。待ち時間は設けず、確認が済みしだい移動する |
...
| 共有時のカード | Discord や X に貼ったときの表示をリンクごとに選べる（転送先のカードを見せる／隠す／内容を指定する）。X に貼ったときの見え方をフォームで確認できる。パスワード保護つきは常に隠す |
| ダッシュボード | 発行履歴（管理者は全URL一覧）を短縮URL・発行日・クリック数・有効期限で並べ替えられる |
...
| カスタマイズ | サイト名・キャッチコピー・運営者名、メインカラー・ダークモード・書体・サービスアイコン（ファビコン）、固定ページの URL を管理画面で変更。利用規約・プライバシーポリシーは Markdown で編集でき、実装に沿ったテンプレートを読み込める |
| 限定モード | 管理者と許可したユーザーだけが発行・ダッシュボードを使える。ほかの人には「このドメインについて」（Markdown で編集）を表示するか、別の URL へ移動する。発行済みの短縮URLは誰でも開け、許可した人が発行したものは安全確認を省いてすぐに移動する |
| 自動アップデート | 1日1回 GitHub Releases を確認し、バックアップ（3世代）→ 更新 → マイグレーション → ヘルスチェック。失敗したら自動で戻して Discord に通知。管理画面の「今すぐ更新する」ですぐにも実行できる。zip を手作業で上書きした場合も、テーブルの変更は次のアクセスで自動適用 |
...
7. 必要に応じて、「管理」の「外部サービス」タブで悪意URLチェック・reCAPTCHA を、「アップデート」タブの「詳細設定」で通知先の Discord Webhook を設定する（自動アップデートは設定しなくても動く）
...
| 自動アップデート | 不要（更新元 `choko1229/URL-Shortener` は設定済み。非公開リポジトリから更新する場合のみトークン） | 「管理」の「アップデート」タブの「詳細設定」 | 既定で有効。無効にもできる |
| 管理者への通知 | Discord Webhook URL | 「管理」の「アップデート」タブの「詳細設定」（「テスト通知を送る」で確認できる） | 通知しない（ログには残る） |
...
- 翌日を待たずに更新したいときは、「管理」の「アップデート」タブの **「今すぐ更新する」** で同じ処理を実行できる（自動アップデートが無効でも実行する）。
- 有効・無効、GitHub のトークン、通知先の Webhook は同じタブの「詳細設定」（通常は閉じている）で設定する。
- 更新元は画面からは変更しない。フォークした場合は `.env` の `SHORTENER_UPDATE_REPOSITORY`（`所有者/リポジトリ名`）を変更する。
- 配布用 zip を手作業で上書きした場合も、テーブルの変更（マイグレーション）は更新後の最初のアクセスで自動で適用する（`SchemaUpdater`。ロックを取り、失敗したら 10 分後に再試行）。
...
| `docs/customize.md` | カスタマイズ（サイト名・見た目・限定モード・法的ページ・ページのURL・お問い合わせ） |
| `docs/services.md` | 外部サービス（Discord・Safe Browsing・reCAPTCHA・国の判定・通知） |
| `docs/operations.md` | 運用（CSV 取り込み・管理者による編集・定期処理・自動アップデート・バックアップ） |
| `docs/api.md` | API |
...
見た目はアプリ本体のデザイントークン（`resources/css/app.css`）に合わせ、`docs/_includes/head_custom.html` の CSS で配色・書体・角丸を上書きしています（ロゴは `title.html`、ファビコンは `favicon.html`）。
...
| 種類 | 置き場所 | 例 |
|---|---|---|
| 環境ごとに異なる値 | `.env` | `APP_KEY`、DB 接続情報、サブドメイン、実行ファイルの場所、更新元（`SHORTENER_UPDATE_REPOSITORY`）、ドキュメントの URL（`SHORTENER_DOCS_URL`） |
| 業務ルールの初期値 | `config/shortener.php` | 月間発行上限、レート制限、未ログイン時の最大有効期限、ロックアウト |
| 管理画面で変更する値 | `app_settings` テーブル（初期値を上書き。機密値は暗号化） | サイト名・運営者名、見た目（配色・ダークモード・書体・アイコン）、公開範囲（限定モード）、ページのURL（`site.paths`）、Discord・Safe Browsing・reCAPTCHA のキー、GitHub トークン、Webhook URL |
| 設置した人が書く文章 | `site_pages` テーブル（Markdown） | 利用規約、プライバシーポリシー、このドメインについて（限定モード） |
| アップロードした画像 | `storage/app/private/branding/`（アプリ経由で `/_icon` として配信） | サービスアイコン |
...
│   ├── Middleware/         # セキュリティヘッダー、セットアップ画面への誘導、テーブルの自動更新、限定モード、API キー認証、アクセス時の定期処理
│   └── Requests/           # 入力検証
├── Installer/              # Web インストーラ、テーブルの自動更新（SchemaState / SchemaUpdater）
├── Models/
...
│                           # 見た目（Theme / ColorPalette / SiteIcon）、公開範囲（AccessPolicy）、ページのURL（SitePaths）
...
| 章 | 内容 |
|---|---|
| 2-1 | アクセス時、ランダムコードは大文字小文字を無視、カスタムスラッグは完全一致。カスタムスラッグは既存のランダムコードと大文字小文字違いで重なる値も使えない。使える文字は英数字・`-`・`_`。管理者（API・CSV を含む）は1文字から使える。サイト内のページと同じパスは管理者でも使えない（ページの URL を変えれば使える） |
| 2-2 | 予約語は大文字小文字を無視して判定。アプリのパス・フォルダと衝突する語（`install` `build` `vendor` など）を初期リストに追加 |
| 2-3 | 未ログイン時の既定の有効期限は 30 日。「期限が近い」表示は残り 3 日以内 |
| 2-4 | カスタムスラッグを編集した場合も、変更前のコードは欠番として再利用しない。編集はログインユーザーの自分のリンク（管理者は全リンク）。管理者は元URL・有効期限・発行者も変更できる |
...
| 2-7 | 危険 → 停止、確認不能 → 警告して利用者が選択。安全ならすぐに移動する（待ち時間なし）。限定モードでは、管理者・許可したユーザーが発行したリンクは確認しない |
...
| 7-2 | GitHub トークンは要件どおり暗号化して DB に保存。ただし DB 接続情報は DB 自体に置けないため `.env` に保存する。更新元のリポジトリは画面では変更せず `.env` で指定する |
| 追加 | 限定モード（管理者と許可したユーザーだけが発行・ダッシュボードを使える。ほかの人には説明ページを表示、または別の URL へ移動） |
| 追加 | 管理者による CSV でのまとめて発行（最大 500 行。月間上限・レート制限・予約語・文字数の設定は適用せず、URL の形式・重複・自サイトの URL などは検証。問題のある行だけ飛ばす） |
| 追加 | 見た目の設定（メインカラー・ダークモード・書体・サービスアイコン）、固定ページの URL の変更 |

## トラブルシューティング

よくある症状と対処は、ドキュメントの「[困ったときは](https://choko1229.github.io/URL-Shortener/troubleshooting.html)」にまとめています（限定モード・サイト設定・自動アップデートの失敗など）。ここには主なものだけを載せます。
...
| 自動アップデートが動かない | 「アップデート」タブの「前回の定期処理」と、「詳細設定」の有効化を確認する。実行履歴にエラー内容が残る。サーバーが自分自身へ接続できない環境（PHP-FPM 以外）では、「今すぐ更新する」で手動実行するか cron を登録する |
| 自動アップデートが「zip を開けません」で失敗する | v26.9.3 以前の不具合。v26.9.4 以降の配布用 zip から `app/Services/Update/GitHubReleaseClient.php` だけをアップロードして上書きし、「今すぐ更新する」を押す |
| 限定モードでダッシュボードを開くと説明ページが出る | ログインしていない人には説明ページを出す。右上の「Discordでログイン」か `dash.example.com/login` からログインする |
...
- 実サーバー（kagoya）では、公開フォルダ直下への設置（`.htaccess`）、アクセス時の定期処理、国判定データの自動取得、テーブルの自動更新は動作を確認済み。zip による自動アップデートの一連の流れ（v26.9.4 の修正後のダウンロード → 入れ替え → ロールバック）と、`mysqldump` の利用は未確認（requirements.md 8 章の未確認事項）

## ライセンス

[MIT License](LICENSE) で公開しています。フォークして自分のサービスとして運用することも、改変して再配布することもできます。その場合は、`.env` の `SHORTENER_UPDATE_REPOSITORY` を自分のリポジトリ（`所有者/リポジトリ名`）に変更してください（非公開リポジトリなら、「アップデート」タブの「詳細設定」で GitHub のトークンも設定します）。
