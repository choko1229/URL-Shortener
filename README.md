...
| サブドメイン | 役割 |
|---|---|
| `chok.ooo` | トップページ（発行フォーム）／短縮URLへのアクセス受付（`chok.ooo/abc1234`）／削除用トークンによる削除（`/delete`） |
...
| 利用制限 | ログイン: 60件/月・5回/分、未ログイン: 5件/月・3分に1回（IP 単位）。未ログインの発行は reCAPTCHA v3 で確認 |
...
| 管理者 | 全URLの管理・削除、ユーザーの管理者昇格、予約語の追加・削除、APIキーの発行、アップデート管理、予約語でもカスタムスラッグにできる |
...
- キー類は APP_KEY で暗号化してデータベースに保存します（reCAPTCHA のサイトキーは公開値のため平文）。
