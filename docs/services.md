...
「管理」の「外部サービス」タブ（管理者のみ）で設定します（管理者への通知だけは「アップデート」タブの「詳細設定」）。キー類は `APP_KEY` で暗号化してデータベースに保存し、画面には表示しません。
...
- 「外部サービス」で、使用中のデータの版・取得日時を確認できます。待たずに取得したいときは「今すぐ取得する」（取得済みなら「今すぐ更新を確認する」）を押します。
...
「管理」の「アップデート」タブの **「詳細設定」**（通常は閉じています）で Webhook URL を設定します。設定後は同じタブの「テスト通知を送る」で届くか確かめられます。次のときに通知します。
