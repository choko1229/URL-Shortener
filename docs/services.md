...
転送のたびに確認し、結果は3日間キャッシュします。安全ならすぐに移動し、危険と判定されたら転送を中止します。確認できなかった場合は警告を表示して、利用者が選べるようにします。確認するのは転送先の URL だけで、訪問者の IP アドレスは送りません。

[限定モード](customize.html#限定モード自分や身内だけで使う)では、管理者と許可したユーザーが発行した短縮URLは確認せず、すぐに移動します。
