...
管理者が登録されると、この設定画面は使えなくなります（以後は管理画面の「外部サービス」で変更します）。

## 5. 仕上げ

- [カスタマイズ](customize.html): サイト名・運営者名、利用規約・プライバシーポリシー
- [外部サービス](services.html): 悪意URLチェック、スパム対策
- [運用](operations.html): 自動アップデートの設定
